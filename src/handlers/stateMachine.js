...
async function handleFlow(session, parsed) {
  ...
  switch (result.action) {
    ...
    case 'answer': {
      ...
      // "Browse all software" is navigation, not a lead answer — leave
      // the flow and open the marketplace website instead of advancing.
      if (typeof step.exitAction === 'function') {
        const action = step.exitAction(result.value);
